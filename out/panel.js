...
class OpenCodePanel {
    constructor(context) {
        ...
        this.client = new opencode_1.OpenCodeClient();
    ...
    removeFileContext(filePath) {
        this.session.contextFiles = this.session.contextFiles.filter((f) => f !== filePath);
        this.post({ type: "updateFiles", files: this.session.contextFiles });
    }
    async pickFiles() {
    ...
    async handleSend(text) {
        ...
        let fullContent = "";
        await this.client.send(text, this.session.contextFiles, (chunk) => {
            fullContent += chunk;
...
exports.OpenCodePanel = OpenCodePanel;
function getNonce() {
    let text = "";
